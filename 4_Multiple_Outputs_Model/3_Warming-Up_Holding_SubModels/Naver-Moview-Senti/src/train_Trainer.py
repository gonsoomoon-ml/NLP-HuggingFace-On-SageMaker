...
def parser_args():
    ...
    parser.add_argument("--disable_tqdm", type=lambda x: (str(x).lower() == 'true'), default=True)        
    ...
    parser.add_argument("--logging_dir", type=str, default=os.environ["SM_OUTPUT_DATA_DIR"])
    ...
    parser.add_argument('--is_evaluation', type=lambda x: (str(x).lower() == 'true'), default=True)    
    parser.add_argument("--eval_ratio", type=float, default=0.2)       
    parser.add_argument("--is_test", type=lambda x: (str(x).lower() == 'true'), default=True) 
    parser.add_argument("--test_data_dir", type=str, default=os.environ["SM_CHANNEL_TEST"])    
    parser.add_argument("--use_subset_train_sampler", default=True, type=lambda x: (str(x).lower() == 'true'))
